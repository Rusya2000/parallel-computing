...
int main(int argc, char* argv[]) {
	...
	if (size % 2 == 1) {
		...
		if (dms[0] * dms[1] != size) {
			printf( "Incompatible dimensions: %i x %i != %i\n",
				dms[0], dms[1], size);
			MPI_Finalize();
			exit(EXIT_FAILURE);
		}
		...
		for (int i = 0; i < size; i++) {
			...
			for (int j = 0; j < size; j++) {
				if (rank == i && near[0] != -1 && near[1] != -1) {
					MPI_Send(&arr[0][j], 1, column_type, near[j], arr[0][j], MPI_COMM_WORLD);


				}

				if (rank == j && near[2] != -1 && near[3] != -1) {
					MPI_Send(&arr[j][0], 1, column_type, near[j], arr[j][0], MPI_COMM_WORLD);
